...
def select_columns(my_table, col_indices):
    ...
    for item in my_table:
        modified_item = []
        for index in range(len(my_table[0])):
            if index in col_indices:
                modified_item.append(item[index])
        modified_table.append(modified_item)
...
def sort_by_column(my_table, col_idx):
    ...
    """
    my_table.sort(key = lambda my_table: float(my_table[col_idx]), reverse=True)
...
##    # Load our file "cancer_risk_trimmed_solution.csv" and compare with your solution
    ...
    print(trimmed_risk_solution == trimmed_risk_table )
