...
def get_county_attributes(svg_file_name):
    ...
    path_strings = [(path.getAttribute('d'), path.getAttribute('id')) for path in doc.getElementsByTagName('path')]
    doc.unlink()
    return path_strings
...
def get_boundary_coordinates(boundary_data):
    ...
    """
    list_of_coordinates = []
##    rough_list = boundary_data.split(" ")
##    for item in rough_list:
##        if 'L'!= item and 'M' !=item and 'z'!= item:
##            refined_item = item.split(',')
####            if 'M' in refined_item[1]:
####                converted_y_coordinate = ""
####                y_coordinate = refined_item[1]
####                converted_y_coordinate = y_coordinate[:len(y_coordinate) - 1]
####                convert_coordinates = (float(refined_item[0]),float(converted_y_coordinate))
####                
####            else:
##            convert_coordinates = (float(refined_item[0]),float(refined_item[1]))
##            list_of_coordinates.append(convert_coordinates)
##            
    rough_list = boundary_data.split('M')
    rough_list1 = []
    rough_list2 = []
    rough_list3 = []
    for item in rough_list:
        new_items = item.split('L')
        for n_items in new_items:
            rough_list1.append(n_items)
    
    for item in rough_list1:
        new_items = item.split('z')
        for n_items in new_items:
            rough_list2.append(n_items)
            
    for item in rough_list2:
        new_items = item.split(" ")
        for n_items in new_items:
            if n_items!= "":
                rough_list3.append(n_items)
        
    for item in rough_list3:
            refined_item = item.strip()
            refined_item1 = refined_item.split(',')
            convert_coordinates = (float(refined_item1[0]),float(refined_item1[1]))
            list_of_coordinates.append(convert_coordinates)
...
def process_county_attributes(svg_file_name, csv_file_name):
    ...
    openfile = open(csv_file_name, "wt", newline="")
    writer = csv.writer(openfile, delimiter=',')
    list_of_attributes = get_county_attributes(svg_file_name)
    for item in list_of_attributes:
        FIPS_code = item[1]
        boundary_data = item[0]
        boundary_coordinates = get_boundary_coordinates(boundary_data)
        centre_coordinates = compute_county_center(boundary_coordinates)
        row = [FIPS_code,centre_coordinates[0], centre_coordinates[1]] 
        writer.writerow(row)
        
    # Ouput CSV file
    print(len(csv_file_name))
...
#county_attributes = get_county_attributes("USA_Counties_2014.svg")
#print(get_boundary_coordinates(county_attributes[0]))
